...
from helpers_Sod_shock_tube import (store_figure,
                     load_or_compute,
                     autocorrelation,
                     wasserstein_distance,
                     DATA_DIR)

sys.path.append("../Experiments/Sod_shock_tube")
from rusanov import RusanovFVM
from hllc import EulerHLLC
...
class PerturbedSodIC:
    """
    f(x) = [1, 0, 1] + params[:3]       if x <= 0.5
           [0.125, 0, 0.1] + params[3:] if x > 0.5 
    """
    l_unperturbed = np.array([1, 0, 1])
    r_unperturbed = np.array([0.125, 0, 0.1])

    def __init__(self, params):
        assert len(params) == 6, ""
        self.jump = 0.5
        self.left = self.l_unperturbed + params[:3]
        self.right = self.r_unperturbed + params[3:]
    ...
    def is_valid(params):
        ...
        Namely it returns False if the inital density or pressure
        is negative.
        """
        if (0 > params[0] + PerturbedSodIC.l_unperturbed[0] or
            0 > params[2] + PerturbedSodIC.l_unperturbed[2] or
            0 > params[3] + PerturbedSodIC.r_unperturbed[0] or
            0 > params[5] + PerturbedSodIC.r_unperturbed[2]):
            print(f"rejected {params}")
            return False
        return True


class PerturbedAdrianSodIC:
    """
    f(x) = [1, 0, 1] + [params[0], 0, params[1]]       if x <= 0.5
           [0.125, 0, 0.1] + [params[2], 0, params[3]] if x > 0.5
    """
    l_unperturbed = np.array([1., 0., 1.])
    r_unperturbed = np.array([0.125, 0, 0.1])
    ...
    def __init__(self, params):
        assert len(params) == 4, ""
        self.jump = 0.5
        left_pertubation = np.array([params[0], 0, params[1]])
        right_pertubation = np.array([params[2], 0, params[3]])
        self.left = self.l_unperturbed + left_pertubation
        self.right = self.r_unperturbed + right_pertubation
    ...
    def is_valid(params):
        """Return True if u is a valid initial condition.
        Namely it returns False if the inital density or pressure is negative
        """
        if (0 > params[0] + PerturbedSodIC.l_unperturbed[0] or
            0 > params[1] + PerturbedSodIC.l_unperturbed[2] or
            0 > params[2] + PerturbedSodIC.r_unperturbed[0] or
            0 > params[3] + PerturbedSodIC.r_unperturbed[2]):
            print(f"rejected {params}")
            return False
        return True


class PerturbedSimpleSodIC:
    """
    f(x) = [1, 0, 1] + [params[0], 0, 0]       if x <= 0.5
           [0.125, 0, 0.1] + [params[1], 0, 0] if x > 0.5
    """
    l_unperturbed = np.array([1., 0., 1.])
    r_unperturbed = np.array([0.125, 0, 0.1])
    ...
    def __init__(self, params):
        assert len(params) == 2, ""
        self.jump = 0.5
        left_pertubation = np.array([params[0], 0, 0])
        right_pertubation = np.array([params[1], 0, 0])
        self.left = self.l_unperturbed + left_pertubation
        self.right = self.r_unperturbed + right_pertubation
    ...
    def is_valid(params):
        """Return True if u is a valid initial condition.
        Namely it returns False if the inital density is negative
        """
        if (0 > params[0] + PerturbedSodIC.l_unperturbed[0] or
            0 > params[1] + PerturbedSodIC.r_unperturbed[0]):
            print(f"rejected {params}")
            return False
        return True
...
class HLLCMCMC:
    def __init__(self, model, domain, N, T):
        self.FVM = EulerHLLC(model, domain, N)
        self.T = T
...
class MeasurerMultiD:
    ...
    def __init__(self, measurement_points, measurement_interval, x_values,
        ...
        self.n_x_vals = len(x_values)
        self.dx = x_values[1] - x_values[0]
        m_p = np.asarray(measurement_points, dtype=np.float64)
    ...
    def __call__(self, values):
        assert values.shape[0] == self.n_x_vals, "Provided values don't match x_vals"

        m = np.zeros((self.left_limits.shape[0], values.shape[1]))
        for i in range(self.n_meas):
            left = self.left_limits[i]
            right = self.right_limits[i] - 1

            for j in range(values.shape[1]):
                left_box = (self.x_cell_limits[left] - self.m_p_left[i]) * values[left-1, j]
                right_box = (self.m_p_right[i] - self.x_cell_limits[right]) * values[right, j]
                m[i, j] = (np.sum(values[left:right, j])*self.dx + left_box + right_box)
                m[i, j] *= 10 * self.weights[i]

        # measurement results are expected to be a vector
        return m.flatten()
