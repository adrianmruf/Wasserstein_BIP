...
def show_burgers_riemann():
    ...
    # delta_2 = 0.25
    # sigma = -0.25
    # U, T = r.integrate(lambda x: delta_2 + (1+delta_1-delta_2)*float(x<sigma),1.,False)
    # np.savetxt('Burgers_ground_truth_ID.txt',U[:,0])
    # np.savetxt('x.txt',r.x[1:-1])

    for i in np.arange(-0.2,0.3,0.1):
        for j in np.arange(-0.2,0.3,0.1):
            for k in np.arange(-0.5,0.1,0.1):
                U, T = r.integrate(lambda x: j+(1-2*j)*float(x < k), 1, False)
    ...
    # U, T = r.integrate(lambda x: float(x < 0), 0.5, False)
