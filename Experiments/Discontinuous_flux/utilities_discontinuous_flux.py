...
sys.path.append("../Experiments/Discontinuous_flux")
from FVM_discontinuous import FVM
...
class FVMObservationOperator:
    ...
    of a FVM
    """
    def __init__(self, IC, fluxL, prior_means, integrator, measurement_operator):
        """
    ...
    def __call__(self, u):
        return self.meas_op(self.integrator(self.IC(self.u_0 + u), self.fluxL(self.u_0 + u)))


class PerturbedRiemannIC:
    """
    u_0(x) = 0.5 + delta_1 if x < -0.5
             2             if x > -0.5
    """
    def __init__(self, params):
        self.left = 0.5 + params[0]
        self.right = 2.
        self.jump = -0.5
...
class PerturbedTransportFlux:
    """
    f_L(x) = (1 + sigma) * x
    """
    def __init__(self,params):
        self.speed = 1 + params[1]
...
class MCMC:
    ...
    """
    def __init__(self, fluxR, fluxRInverse, cfl, domain, N, T):
        self.FVM = FVM(domain, N)
        # self.fluxL = fluxL
        self.fluxR = fluxR
        self.fluxRInverse = fluxRInverse
        self.cfl = cfl
        self.T = T

    def __call__(self, IC, fluxL):
        # u as in "final state u", not "MCMC state u"
        self.FVM.integrate(IC, fluxL, self.fluxR, self.fluxRInverse, self.cfl, self.T)
        return self.FVM.u_new[-1,:]
...
class Measurer:
    ...
    def __init__(self, measurement_points, measurement_interval, x_values):
        ...
        m_p = np.asarray(measurement_points, dtype=float)
    ...
    def __call__(self, values):
        ...
        m = np.empty_like(self.left_limits, dtype=float)
...
#     @staticmethod
#     def IC(x):
#         return np.sin(np.pi*x)#0.85 - 0.6 * float(x < 1)
