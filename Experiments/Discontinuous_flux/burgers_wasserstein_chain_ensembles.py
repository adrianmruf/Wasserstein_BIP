...
sys.path.append("../Discontinuous_flux")
from utilities_discontinuous_flux import (FVMObservationOperator,
                                          PerturbedRiemannIC,
                                          PerturbedTransportFlux,
                                          MCMC,
                                          Measurer,
                                          store_figure,
                                          load_or_compute,
...
class Settings:
    ...
    class Simulation:
        class IC:
            ...
            delta = 0.
            sigma = 0.
            ground_truth = np.array([delta, sigma])

        domain = (-1.,1.)
        N_gridpoints = 128
        T_end = 1.

        # fluxL = DiscontinuousFlux.fluxL
        fluxR = DiscontinuousFlux.fluxR
        fluxRInverse = DiscontinuousFlux.fluxRInverse
        cfl = 0.4

        @staticmethod
        def get_xvals():
            return MCMC(None, None, None,
                        Settings.Simulation.domain,
                        Settings.Simulation.N_gridpoints,
                        0).FVM.x[1:-1]

    class Measurement:
        points = [-0.5, 0.1, 0.3, 0.5, 0.7, 0.9]
        interval = 0.15
...
def create_integrator():
    return MCMC(Settings.Simulation.fluxR,
                Settings.Simulation.fluxRInverse,
...
def create_mcmc_sampler():
    ...
    fluxL_true = PerturbedTransportFlux(Settings.Simulation.IC.ground_truth)
    ...
    ground_truth = create_measurer()(create_integrator()(IC_true, fluxL_true))
    # print(ground_truth)
